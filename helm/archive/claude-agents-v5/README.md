...
The chart includes an optional in-cluster registry so nodes never depend on WSL being open to pull images.

### Why in-cluster

- Cluster boots and runs independently — no WSL dependency
- Fast pulls — images are on the local network
- Self-contained — everything the pipeline needs lives in the cluster

### Push an image from WSL

```bash
# Build your agent image
podman build -t claude-agent:latest .

# Push to the in-cluster registry via NodePort
podman push 192.168.100.10:30500/claude-agent:latest --tls-verify=false

# Verify
curl http://192.168.100.10:30500/v2/_catalog
# Returns: {"repositories":["claude-agent"]}
```

### Tell containerd to trust it

Run on each node after install:

```bash
ssh pdawson@192.168.100.10 "sudo bash -c 'cat >> /etc/containerd/config.toml << EOF

[plugins.\"io.containerd.grpc.v1.cri\".registry.mirrors.\"192.168.100.10:30500\"]
  endpoint = [\"http://192.168.100.10:30500\"]
EOF
