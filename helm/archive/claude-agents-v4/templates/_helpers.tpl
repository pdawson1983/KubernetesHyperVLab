...
{{/*
Registry address — used in agent image references
Returns the in-cluster registry address if enabled, otherwise global image repository
Usage: include "claude-agents.registryAddress" .
*/}}
{{- define "claude-agents.registryAddress" -}}
{{- if .Values.registry.enabled -}}
{{ include "claude-agents.fullname" . }}-registry.{{ .Values.global.namespace }}.svc.cluster.local:5000
{{- else -}}
...
{{/*
Full image reference for an agent image
...
{{ include "claude-agents.registryAddress" .root }}/{{ .name }}:{{ .root.Values.global.image.tag }}
{{- else -}}
